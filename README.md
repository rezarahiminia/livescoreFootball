...
The public API never calls ESPN or another upstream provider during a customer request. A separate listener project fetches upstream data and writes normalized documents to MongoDB; this service only reads and publishes those documents.

## Product transition

This project is moving away from its original World Cup-only scope. The legacy
World Cup service will be taken out of public access and will not receive new
product features. The replacement is a country-based club football service
with one stable API and data model across all supported leagues.

The transition is intentionally incremental:

| Phase | Scope | Status |
|---|---|---|
| 1 | Retire the public World Cup website, endpoints, documentation, and update jobs | In progress |
| 2 | Launch England and Spain, starting with the Premier League (`eng.1`) and LaLiga (`esp.1`) | First priority |
| 3 | Add leagues from other countries one country at a time | Planned |

Removing public access does not require deleting historical World Cup data.
Legacy collections may be kept temporarily for rollback or archival purposes,
but they are not part of the new public API and must not appear in Swagger or
...
Responses for scoreboard, summary, and plays follow an ESPN-like shape while remaining backed entirely by this project's MongoDB.
...
Swagger endpoints; `/robots.txt` and
`/sitemap.xml` describe the football API rather than the retired tournament
site.
...
- Live soccer routes bypass the application's longer in-process response cache.
...
- [Upstream ESPN field research](ESPN-LIVE-DATA.md)
